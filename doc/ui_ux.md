...
- **Avatars: customizable cartoony animal.** Replaces the single-emoji icon picker. Four independent choices, each from a small fixed set: **animal** (base body/head shape — pick ~6, e.g. bear/cat/fox/panda/tiger/bunny), **hat**, **glasses**, **shirt** (each ~5 options including "none" where it makes sense). Composited as one layered SVG. Full detail at landing-screen/lobby-seat size; a simplified (fewer visible details, but still on-model) version is fine at the small opponent-seat avatar size on the table — legibility at a glance matters more than full fidelity there.
  ...
  - **Builder UI (revised):** a large zoomed-in avatar preview, not a grid of small swatches. Four rows of left/right cycle-arrows positioned at the vertical zone of the avatar each one controls, so the control reads as "attached to" the part it changes: **top** = hat, **upper-middle** = glasses, **lower-middle** = animal (base species — the control sits near the face/jaw even though the choice affects the whole body), **bottom** = shirt. Clicking an arrow steps to the next/previous option in that slot (wrapping around) and the big preview updates immediately — this replaces needing to scan a grid to see what something looks like on your current build.
- **Art direction: anime-inspired, cute.** Applies to every character (avatars) and the game's scenery. Think chibi proportions — larger expressive eyes, soft rounded silhouettes, a little blush/highlight detail — while staying simple flat-shape SVG (gradients/soft shading are fine, no raster textures or external images). This supersedes the plainer flat-cartoon shape language from the first re-theme pass; redraw the avatar pieces (animal/hat/glasses/shirt) and the card faces to match rather than layering a new style on top of the old shapes.
- **Scenery: detailed jungle, not just a floor.** Both the lobby and table screens get the jungle treatment (the first re-theme pass only reached the table) — layered background elements: trees and bushes framing the edges, scattered rocks, a river or water feature somewhere in the scene. Keep it readable: background elements stay visually behind/around the actual UI (seat cards, table, hand, buttons), never competing with it for attention. Still static, no parallax/animation, per the animation budget above.
...
- Username input, avatar builder (see "Avatars" above — no upload, keeps this consistent with the "no persistence" non-goal).
- "Create Lobby" button → `room:create`, then router push to `/room/[code]`.
- "Join by code" input + button → `room:join`.
- On mount, check `sessionStorage` for a saved `{ code, sessionToken }`; if present, attempt `room:rejoin` and redirect straight into the room on success.
